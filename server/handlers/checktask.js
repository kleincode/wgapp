...
module.exports = ({ db }) => ({
  ...
  handler: async ({ body, query, user }, { success, fail, error }) => {
    try {
      const { results } = await db.query("SELECT COUNT(*) AS 'c' FROM tasks WHERE id = ? AND hid = ?", [id, req.user.hid]);

      if (results[0].c < 1) {
        fail("The specified task does not exist.");
      } else try {
        await db.query("UPDATE tasks SET lastExecution = ? WHERE id = ?", [new Date(), id]);
